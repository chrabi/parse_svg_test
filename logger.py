...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    default_config = {
        "log_setup": default_log_config
        # Other settings can be added here
    }
    
    if config_path is None:
        # Place config file in the config directory relative to the script
    ...
    try:
        ...
        # Get log_setup section, create if missing
        log_config = config.get("log_setup", {})
        ...
        # Update the config file if log_setup was missing
        if "log_setup" not in config:
            config["log_setup"] = log_config
            with open(config_path, 'w') as f:
